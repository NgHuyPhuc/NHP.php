...
<body>
    ...
    if (isset($_GET['name'])) {
        ...
        $sql = "SELECT *
        From quanao 
        Where `quanao`.`Tenhh` like '%$name%'";
        $quanaosearch = $db->fetchAll($sql);
    }

    // $sql = "SELECT `quanao`.*, `loaisp`.`ID`,`loaisp`.`TenLoai`
    // from `quanao` , `loaisp` 
    // where `quanao`.`ID`=`loaisp`.`ID` and `quanao`.`Tenhh` like '%$name%' LIMIT 15";
    $sql = "SELECT `quanao`.*, `loaisp`.`ID`,`loaisp`.`TenLoai`
    from `quanao` , `loaisp` 
    ...
    if (isset($_GET['DongGia'])) {
        $DongGia = $_GET['DongGia'];
        $sqlgia = "SELECT * from quanao where `quanao`.`DongGia` <='$DongGia'";
        $quanaogia = $db->fetchAll($sqlgia);
    }

    // $sqlgia="SELECT * from quanao where"
    ?>
    <div class="app">
                    ...
                    <div class="grid-row">
                        ...
                        <div class="grid__column-10 product-main">
                            ...
                            <div class="home-product">
                                <div class="grid-row">
                                    <?php foreach ($quanaosearch as $index => $item) : ?>
